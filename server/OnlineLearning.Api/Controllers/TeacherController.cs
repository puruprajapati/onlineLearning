...
namespace OnlineLearning.Api.Controllers
{
    ...
    public class TeacherController : ControllerBase
    {
        ...
        {
            var userContext = HttpContext.GetUserContext();
            var classData = _mapper.Map<TeacherViewModel, Teacher>(request);
            classData.ModifiedByUserId = userContext.Id;
            var result = await _teacherService.UpdateAsync(id, classData);

            if (!result.Success)
                return BadRequest(new ErrorResource(result.Message));

            var resultViewModel = _mapper.Map<Teacher, TeacherViewModel>(result.Teacher);
            return Ok(resultViewModel);
        }
    }
}
