...
namespace OnlineLearning.Api.Controllers
{
    ...
    public class SubjectController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAll([FromQuery] BaseParameter baseParameter)
        {
            ...
            var resultViewModel = _mapper.Map<IEnumerable<Subject>, IEnumerable<SubjectViewModel>>(results);
            var metadata = new
            {
                results.TotalCount,
                results.PageSize,
        ...
        [HttpPost]
        public async Task<IActionResult> CreateAsync([FromBody] SubjectViewModel newData)
        {
            var userContext = HttpContext.GetUserContext();
            var mappedData = _mapper.Map<SubjectViewModel, Subject>(newData);
            var response = await _service.CreateAsync(mappedData, userContext);
            if (!response.Success)
            {
                return BadRequest(new ErrorResource(response.Message));
            }
            var addSession = _mapper.Map<Subject, SubjectViewModel>(response.Subject);
            return Ok(addSession);
        }

        [HttpPut("{id}")]
        public async Task<IActionResult> PutAsync(Guid id, [FromBody] SubjectViewModel request)
        {
            var userContext = HttpContext.GetUserContext();
            var data = _mapper.Map<SubjectViewModel, Subject>(request);
            var result = await _service.UpdateAsync(id, data, userContext);

            if (!result.Success)
                return BadRequest(new ErrorResource(result.Message));

            var resultViewModel = _mapper.Map<Subject, SubjectViewModel>(result.Subject);
            return Ok(resultViewModel);
        }
