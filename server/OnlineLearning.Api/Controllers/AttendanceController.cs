...
namespace OnlineLearning.Api.Controllers
{
    ...
    public class AttendanceController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAll([FromQuery] BaseParameter baseParameter)
        {
            ...
            var resultViewModel = _mapper.Map<IEnumerable<Attendence>, IEnumerable<AttendanceViewModel>>(results);
            var metadata = new
            {
                results.TotalCount,
                results.PageSize,
        ...
        [HttpPost]
        public async Task<IActionResult> CreateAsync([FromBody] AttendanceViewModel newData)
        {
            var userContext = HttpContext.GetUserContext();
            var mappedData = _mapper.Map<AttendanceViewModel, Attendence>(newData);
            var response = await _service.CreateAsync(mappedData, userContext);
            if (!response.Success)
            {
                return BadRequest(new ErrorResource(response.Message));
            }
            var addSession = _mapper.Map<Attendence, AttendanceViewModel>(response.Attendence);
            return Ok(addSession);
        }

        [HttpPut("{id}")]
        public async Task<IActionResult> PutAsync(Guid id, [FromBody] AttendanceViewModel request)
        {
            var userContext = HttpContext.GetUserContext();
            var data = _mapper.Map<AttendanceViewModel, Attendence>(request);
            var result = await _service.UpdateAsync(id, data, userContext);

            if (!result.Success)
                return BadRequest(new ErrorResource(result.Message));

            var resultViewModel = _mapper.Map<Attendence, AttendanceViewModel>(result.Attendence);
            return Ok(resultViewModel);
        }
