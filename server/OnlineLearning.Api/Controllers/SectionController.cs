...
namespace OnlineLearning.Api.Controllers
{
    ...
    public class SectionController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAll([FromQuery] BaseParameter baseParameter)
        {
            ...
            var resultViewModel = _mapper.Map<IEnumerable<SectionDetail>, IEnumerable<SectionViewModel>>(results);
            var metadata = new
            {
                results.TotalCount,
                results.PageSize,
        ...
        [HttpPost]
        public async Task<IActionResult> CreateAsync([FromBody] SectionViewModel newData)
        {
            var userContext = HttpContext.GetUserContext();
            var mappedData = _mapper.Map<SectionViewModel, SectionDetail>(newData);
            var response = await _service.CreateAsync(mappedData, userContext);
            if (!response.Success)
            {
                return BadRequest(new ErrorResource(response.Message));
            }
            var addSession = _mapper.Map<SectionDetail, SectionViewModel>(response.SectionDetail);
            return Ok(addSession);
        }

        [HttpPut("{id}")]
        public async Task<IActionResult> PutAsync(Guid id, [FromBody] SectionViewModel request)
        {
            var userContext = HttpContext.GetUserContext();
            var data = _mapper.Map<SectionViewModel, SectionDetail>(request);
            var result = await _service.UpdateAsync(id, data, userContext);

            if (!result.Success)
                return BadRequest(new ErrorResource(result.Message));

            var resultViewModel = _mapper.Map<SectionDetail, SectionViewModel>(result.SectionDetail);
            return Ok(resultViewModel);
        }
